...
watch(audios.value, (newValue) => {
    const newAudio = newValue.filter(x => !ap.value.list.audios.map((e: any) => e.id).includes(x.id));
    ap.value.list.add(newAudio);
})
watch(playingAudio, (newValue) => {
    const pIndex = ap.value.list.audios.findIndex((x: any) => x.id == newValue.id);
    ap.value.list.switch(pIndex)
})
</script>